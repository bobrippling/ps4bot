...
def game_rankdelta(game, players):
    return elo.ranking_delta_for_game(game, players, None)

class TestPS4Elo(unittest.TestCase):
    def test_expected_score(self):
        def calc_score(ranking, other_ranking):
            return round(elo.expected_score(ranking, other_ranking), 4)
    ...
    def test_ranking_delta(self):
        rank_delta = elo.ranking_delta
        self.assertEqual(rank_delta(1500, 1500, elo.Result.win, 32), 16)
        self.assertEqual(rank_delta(1500, 1500, elo.Result.loss, 32), -16)
        self.assertEqual(rank_delta(2000, 1500, elo.Result.win, 32), 2)
        self.assertEqual(rank_delta(1000, 1500, elo.Result.win, 32), 30)
        self.assertEqual(rank_delta(3000, 1000, elo.Result.win, 32), 1)
        self.assertEqual(rank_delta(3000, 1000, elo.Result.loss, 32), -32)
        self.assertEqual(rank_delta(1000, 3000, elo.Result.loss, 32), -1)
    ...
    def test_calculate_scrub_ranking(self):
        scrub_mod = elo.calculate_scrub_modifier
        ...
        player1 = elo.Player(1)
        player2 = elo.Player(2)
        player3 = elo.Player(3)
        player4 = elo.Player(4)
        game = elo.Game(teams, 0, scrubs)
    ...
    def test_calculate_ranking(self):
        def calc_ranks(game):
            return elo.calculate_rankings(game, None)
        Game = elo.Game
